<x-teacher.layout :yearGroups="$yearGroups" title="Teacher Dashboard">
  <div class="space-y-3">
    ...
    @foreach(($yearGroups ?? []) as $group)
      <div class="grid grid-cols-12 gap-3 rounded-md bg-[#755f541e] p-3">
        <div class="col-span-12 sm:col-span-6 flex flex-col justify-center px-2">
          @php
           if ($group['year'] == 0) {
        ...
        <a href="{{ url('/teacher/classes/'.$group['slug'].'/view') }}"
           class="col-span-12 sm:col-span-2 flex items-center justify-center rounded-lg bg-[#755f54b6] px-6 py-3 text-sm font-bold text-white hover:bg-primary transition text-center">
          View
        </a>
